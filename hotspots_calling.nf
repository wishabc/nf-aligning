...
process call_hotspots {
	...
	output:
	    tuple val(id), path(spot), path("nuclear.cleavage.total"), path("nuclear.density.bw"), path("nuclear.hotspot2.info"), path("nuclear.cutcounts.starch"), path("nuclear.allcalls.starch"), emit: extra
        tuple val(id), path("${id}.hotspots.fdr0.001.starch"), path(name), emit: point1pr
        tuple val(id), path(hotspot1pr), path(peaks1pr), emit: one_pr
        tuple val(id), path(hotspot5pr), path(peaks5pr), emit: five_pr
