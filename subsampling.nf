...
process spot_score {
    ...
    output:
        tuple val(sample_id), path("r1.spot.out")
...
process subsample_with_pairs_frac {
    ...
    input:
        tuple val(sample_id), path(cram_file), path(cram_file_index), val(frac)
    
    output:
        tuple val(sample_id), path(name), path("${name}.bai")

    script:
    name = "${sample_id}.subsampled_pairs.bam"
    """
